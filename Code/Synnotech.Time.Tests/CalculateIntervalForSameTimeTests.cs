...
{
    public static class CalculateIntervalForSameTimeTests
    {
        private static readonly DateTime StartTime = new (1, 1, 1, 4, 15, 0, DateTimeKind.Local);
        
        [Theory]
        [MemberData(nameof(NonSpecificData))]
        public static void CalculateIntervalForSameTimeInNonSpecificCultureScenario(DateTime now, TimeSpan expected) =>
        ...
        {
            var actualTimeSpan = now.CalculateIntervalForSameTime(StartTime);
